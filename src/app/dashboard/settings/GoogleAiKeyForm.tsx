...
export function GoogleAiKeyForm() {
  ...
  useEffect(() => {
    ...
    fetch("/api/user/settings")
      ...
      .then((data) => {
        if (cancelled) return;
        setMasked(data.googleAiStudioApiKey ?? null);
        setHasKey(!!data.hasGoogleAiStudioApiKey);
      })
      .catch(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/user/settings", {
        ...
        body: JSON.stringify({
          googleAiStudioApiKey: apiKey.trim() || null,
        }),
      });
      if (!res.ok) {
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row sm:items-end">
        <div className="min-w-0 flex-1">
          <label htmlFor="google-ai-key" className="sr-only">
            Google AI Studio API key
          </label>
          <input
            id="google-ai-key"
